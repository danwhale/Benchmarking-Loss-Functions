...
class Net(torch.nn.Module):
    ...
    def lossLaplacianEigenMaps(self,out,A):
       ...
       # dd=torch.device('cpu')
        ...
        yDy = torch.matmul(torch.matmul(out_tr,torch.diag(sum(A.t())).type(torch.FloatTensor).to(dd)),out) - torch.diag(torch.ones(out.shape[1])).type(torch.FloatTensor).to(dd)
