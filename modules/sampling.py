...
class SamplerContextMatrix(SamplerWithNegSamples):
    ...
    def pos_sample(self, batch, **kwargs):
        ...
        if self.loss["C"] == "Adj" and (self.loss["Name"] == "LINE" or self.loss["Name"] == "Force2Vec"):
            ...
            name = f"{self.help_dir}/pos_samples_VERSEAdj_" + self.datasetname + ".pickle"
            if os.path.exists(name):
                with open(name, "rb") as f:
                    pos_batch = pickle.load(f)
            else:
            ...
            if os.path.exists(SimRankName):
                ...
                with open(SimRankName, "wb") as f:
                    pickle.dump(A, f)
            samples_name = f"{self.help_dir}/samples_simrank_" + self.datasetname + ".pickle"
            ...
            alpha = self.alpha
            name_of_file = f"{self.help_dir}/pos_samples_VERSEPPR_" + str(alpha) + "_" + self.datasetname + ".pickle"
...
class SamplerAPP(SamplerWithNegSamples):
    ...
    def pos_sample(self, batch, **kwargs):
        ...
        name_pos_samples = "..data_help/APP_" + self.datasetname + "_" + str(self.alpha) + "_.pickle"
